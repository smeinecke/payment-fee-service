...
from payment_fee.errors import ProviderDataUnavailable, UnknownProvider
from payment_fee.models import ProviderInfo
...
class ProviderRegistry:
    ...
    def get(self, provider_id: str) -> FeeProvider:
        if provider_id in self._errors:
            raise ProviderDataUnavailable(provider_id, self._errors[provider_id])
        try:
    ...
    def infos(self) -> list[ProviderInfo]:
        ...
        return [
            ProviderInfo(
                ...
                market_count=(len(self._providers[provider_id].markets()) if provider_id in self._providers else 0),
